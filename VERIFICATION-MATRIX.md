...
| 四季事件 | `mechanics/50` | `L1`（**四季常式一年各跑一次**、年齡在元月、人口在十月、**蝗害的兩條損失**、**水災連郡裡的人物一起動**：`SAN1_MONTH=3` 把月度對拍的視窗推到四月，淹 3 郡那一趟州郡表與人物表 0 位元組）／`L0`（四種天災的機率與幅度、洪水率的年度變動、`DS:0x679c` 的 43 格基礎表、秋收、進貢）| `internal/game/events.go` | 有 |
