...
| 強化 AI 會留兵防守了 | 完成 | L1 | base | `TestZZGarrisonRatioSweep`、`TestZZAIModesMoveTheBoard` | 先前的強化 AI **只會擴張不會守**：三十六個月打下 38 個郡，而總兵只剩 76,067（`base` 是 166,250）——沒有足夠的兵力防守，再多的土地與金子都是白搭。  三處改動： ① **守不住就先補兵**（`garrisonNeed`：鄰郡最強敵人的守備戰力 × 60%），排在出兵前面； ② **出兵要留得住家**：從最強的開始編隊，但留下的戰力不能低於門檻（先前是「除了一位全部帶走」）； ③ **每郡每月下多道令**——電腦本來就不受那條限制（`docs/mechanics/70-ai` §2.12），先前自我設限成「要嘛打仗要嘛補兵」。  結果：總兵 76,067 → **149,155**（`base` 的 90%），而最大勢力仍有 12 郡（`base` 只有 4 郡）。門檻 60% 是掃出來的，曲線在 `docs/design/02`。（`docs/design/02`） |
