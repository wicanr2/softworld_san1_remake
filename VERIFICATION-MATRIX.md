...
| **兩版的月度對拍推到四季（十二個視窗）** | `SAN1_MONTH=n` 把視窗推到 n 月底（跨進 n+1 月），`SAN1_PLAGUE=1` 把一個郡擺成必淹必瘟；原版與加強版各六個視窗（預設、3、3＋瘟疫、6、9、12），三張表整月逐位元組比 | **十二個視窗各差 0 個位元組**（2026-09-16，Issue #44）：四季常式兩版逐段同形，加強版獨有的三條（元月忠誠漂移的難度門、挖角的 `RND(1010)` 難度門、繼承之後多一則對白）與兩版共用碼修的一批（元月順序與對白／特效的擲、地震震兵、蝗害不看所屬且土地價值不夾、挖角挑忠誠最低者＋等級加成＋三條寫回路、武器預算扣折扣價、移防看目標郡餘裕、出兵看存的兵士、買米的 8087 精度）在 `docs/mechanics/50` §1.1、`70` §2.13.8、`90` §6.6b；每支 ≈ 35 秒 |
