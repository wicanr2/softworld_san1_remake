...
（`internal/battle/skirmish.go`），盤面丁 108 條鏈含第 24 天一整場對戰
逐條相同；加強版的這一層還沒對（`docs/re/05` §10.9），盤面丁走到第一次
對戰為止。**加強版是同一支本體換一組路標**
...
| 戰場上部隊每天怎麼選動作（九支與對戰子畫面） | 完成 | L1 | base | `TestZZUnitAIDayParity` | 九支的角色、條件與骰序（`docs/re/05` §12.1–12.2，Issue #22／#24）、加強版的改動（§12.5，#28）與選項 7 進的對戰子畫面（§10，#30：版型、起點、每一時刻的判斷式與六處骰、尋路、攻擊、俘虜）都對回原版；`SAN1_NORESYNC=1 TestZZUnitAIDayParity` 五張盤面整場逐條相同（丁含一整場對戰）。還沒對的是加強版的對戰子畫面（§10.9）與子畫面裡單挑的兩擲對白（§13）。（`docs/re/05`、`docs/mechanics/40`、`docs/design/02`） |
