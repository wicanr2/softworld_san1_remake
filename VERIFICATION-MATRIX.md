...
| 項目 | 狀態 | 等級 | 版本 | 核實訊號 | 說明 |
|---|---|---|---|---|---|
| 戰場上部隊每天怎麼選動作還沒對上 | 未完成 | L2 | base | present：docs/mechanics/40-military.md 的「各個選項分別是什麼動作還沒對上」 | **結構解出來了**（`docs/re/05` §12）：入口 `0x29014`，九個選項依序試，動作碼寫在 `es:[0x31a8]`，日層級對照乾淨（決策定出 2 的那幾天各跑一次交戰結算）。  **各個選項分別是什麼動作還沒對上**：九支裡只確認了三支的角色——`0x29e2e` 兜底、`0x29784` 與 `0x29ade` 選對戰；其餘六支在觀測的盤面上沒定過案（敵人不在旁邊時十三次全走到最後一個選項）。  所以 **remake 的戰場 AI 門檻與順序是 remake 自己決定的**（死戰、叫陣、退兵、用計的時機，`docs/design/02`）。要往下走得先把敵人擺到旁邊，讓前面那幾支定案。（`docs/re/05`、`docs/mechanics/40`、`docs/design/02`） |
