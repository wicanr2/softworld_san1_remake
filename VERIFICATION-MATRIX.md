...
| 項目 | 狀態 | 等級 | 版本 | 核實訊號 | 說明 |
|---|---|---|---|---|---|
| 自創君主的名字字模還沒寫進 `BASEPRE` | 未完成 | L0 | base | present：internal/save/save.go 的「自創君主的字模還沒接」 | 原版把自創君主的名字當**造字**（Big5 `A141`–`A14C`），字模存在 `BASEPRE.SVn`（12 個 16×16）。remake 現在存出去的是空字模，原版讀回來那三個字會是空白。  名字固定是「新君主」，而**出貨的 `BASEPRE` 內容正好就是它**（`docs/re/08` §3：六個進度位元組完全相同）——接法是開局時把 `DATA2` 裡那一份帶進來。之後要讓玩家打字改名，字模就從 remake 的字庫取。（`docs/spec/013`、`docs/re/08`） |
...
| 多人（玩家人數那一層）還沒做 | 未完成 | L2 | base | absent：internal/session 的「Players []state[.]FactionID」 | 原版開新局是「時期 → **玩家人數** → 角色 → 難度」（手冊 p.7）。remake 只有單人：`session.Session` 的 `Player` 是一個勢力，月流程也只讓那一個下令。  多人要動的是 session 的回合結構（每個玩家依序下令），不是選單那一層。⚠ **對拍全是單人**，改動要小心別動到單人那條路。（`docs/reference/01-manual-20-mechanics`） |
