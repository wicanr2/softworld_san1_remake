...
| 項目 | 狀態 | 等級 | 版本 | 核實訊號 | 說明 |
|---|---|---|---|---|---|
| 戰場上部隊每天怎麼選動作（九支解出七支） | 未完成 | L1 | base | `TestZZUnitAIOptionRoles` | **九支裡解出七支**（`docs/re/05` §12，`TestZZUnitAIOptionRoles`）：把每一支跑期間叫到的後果常式記下來——交戰結算、計謀判定、天候門、弓箭、畫部隊標記各自是什麼早就解了，所以「這一支叫了誰」直接說出它在做什麼。  \| 選項 \| 角色 \| \|---\|---\| \| 1 `29E78` \| 前置：清動作碼 ＋ 浮點評估 \| \| 2 `29138` \| 前置（無條件跑，沒定過案）\| \| 3 `29344` \| **移動** \| \| 4 `2985C` \| **弓箭** \| \| 5 `29784` \| **用計** \| \| 6 `29C56` \| 靜態看呼叫交戰結算；**沒試到** \| \| 7 `29B82` \| 呼叫 `0x2DEB0`；**沒試到** \| \| 8 `29ADE` \| **對戰** \| \| 9 `29E2E` \| **兜底（休息）** \|  動作碼 `2` ＝ 這一天要打。機率是 `L0`：弓箭 `RND(2)==0`、用計 `RND(3)!=0`、選項 6 `RND(4)==0`、選項 7 `RND(16)==0`。  ⚠ **剩下兩支要換盤面**：選項 5（用計）定案之後就跳過 6 與 7，得找一個用不出計的局面（沒錢或智力不足）。  ⚠ 先前記著「`0x29784` 與 `0x29ade` 會選出對戰」——前者叫的是**計謀判定**不是交戰結算。（`docs/re/05`、`docs/mechanics/40`、`docs/design/02`） |
