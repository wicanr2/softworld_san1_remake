...
| `.MSK` 蓋在哪一張圖上還沒解（格式已解） | 未完成 | L0 | base | present：docs/formats/07-images.md 的「蓋在哪一張圖上還沒解」 | **格式解出來了**：表頭與 `.IMG` 相同（先高後寬），但只有一個平面——一個位元一個像素（`assets.DecodeMask`）。整套素材裡只有一張：`DATA2` 的 `ENDO4.MSK`，640×151，內容是山巒的輪廓。  ⚠ 它與 `ENDO0`–`ENDO3`（結局的朝堂圖）放在一起，**但那張圖裡沒有山**——蓋在哪一張上沒解。用它的是 `DATA0.GRP`／`DATA4.GRP` 那兩支 overlay，主程式的映像裡一次都沒出現 `ENDO`／`.MSK` 這些字串。  主戰場年月那一行的網點**與遮罩無關**，已經解成 `(x+y)%2` 的兩色棋盤（`docs/spec/011` §2）——這一條原本被當成它的 blocker，現在解開了。（`docs/formats/07`、`docs/formats/01`） |
...
| 結局畫面 remake 還沒畫 | 未完成 | L0 | base | absent：internal/ui 的「ENDO」 | 素材找到了（`CONTEXT.md` R56）：`DATA2` 的 `ENDO0`–`ENDO3` 四張 160×336 並排成 640×336 的朝堂圖，加一張 640×151 的遮罩 `ENDO4.MSK`。`REC10L/R`、`REC11L/R` 各 320×336，`UPR00`–`UPR21` 大小不一。  ⚠ **怎麼用沒解**：主程式的映像裡一次都沒出現這些名字，用它們的是 `DATA0.GRP`／`DATA4.GRP` 那兩支 overlay——那兩支是 MZ 執行檔，還沒反組譯。  remake 現在統一之後只在提示列說一句話（`msg.over`）。（`docs/formats/07`、`docs/formats/01`） |
| 開新局比原版少兩層：玩家人數與自創君主 | 未完成 | L2 | base | absent：internal/menu 的「CustomLord」 | 原版是「時期 → 玩家人數 → 角色 → 難度 →（選新君主就做七項設定）→ 密碼」（手冊 p.7／p.14）。remake 的 `internal/menu` 只有「劇本 → 君主 → 難度」，**少了玩家人數與自創君主兩層**。  自創君主的七項在原版字串裡看得到（`0x47941`：`1.年齡` `2.體能` `3.謀略` `4.戰力` `5.魅力` `6.領地`／`新君主出現!!`），手冊說**四項能力共用 100 點**、領地只有一郡且必須是空白郡。存檔那一層 remake 已經做了（`BASEPRE.SV?` 的十二個字模往返）。  ⚠ **卡在同一個地方**：那一段跑在開機鏈第二層（`DATA0.GRP`），主程式的碼段 dump 涵蓋不到——與 `ending-screen`、`msk-masks` 是同一支 overlay。（`docs/reference/01-manual-20-mechanics`、`docs/re/04`） |
