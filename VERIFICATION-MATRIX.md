...
| **一個月的狀態轉移** | 兩邊從同一份盤面各走同一個視窗，逐欄位比 | ⚠ **2026-09-10 起是紅的（差 145）**：起點漂移已修（改由原版的路標定位＋固定亂數），剩下的是一條真的規則缺口——**出兵的錢糧在月底結算之後才從郡搬走**，remake 沒做（`docs/re/05` 末節）。舊起點上曾**差 0 個位元組**（43 個郡、350 位人物、16 個勢力逐格相同；十張分派表的抽亂數次數全部 ±0。`TestZZMonthParity`，2026-09-08）|
...
| 項目 | 狀態 | 等級 | 版本 | 核實訊號 | 說明 |
|---|---|---|---|---|---|
| 槽位溢出：選單與分頁量過了，郡的資料面板與年月直排還沒 | 未完成 | — | — | present：internal/ui/artscreen.go 的「欄名還沒接譯文」 | **2026-09-11 量過的**（`docs/spec/014` §3）： ① 子選單——中文九類與原版字串逐行相同；英文 1／3／7／9 類排不進下面板改畫上面板、8 類（`2.Befriend Afar, Attack Near` 28 格）改蓋內容區；日文九類都在下面板。 ② 十項指令表——十項有一項超過 4 格就整張改畫一倍字（英日文）。 ③ 分頁——表格改成依內容定欄寬（`cells.Columns`）。先前寫死的寬度會截斷：英文 `GovernorGold`、`Floo`、職位被截到六格；中文的將軍列表把將軍的忠誠欄標成「民眾」。現在三個語系在劇本 001 開局的每一個郡、勢力與將軍上都在 68 格內（繁中 60、英文 67、日文 52）。  **還沒做的**：郡的資料面板的欄名是寫死的中文（標籤 ＋ 靠右的數值共 12 格，英文要短欄名）；左側直條的年月在英文下一個字母一列；戰場畫面的部隊表（`battlescreen.go` 還用寫死寬度的 `Pad`）沒量。（`CLAUDE.md §3.3`、`docs/spec/014`） |
