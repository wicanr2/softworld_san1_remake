...
type Unit struct {
	...
	// RetreatTo 是退兵時挑的去處（郡編號，0 ＝ 沒退過，Issue #101）。
	//
	// ⚠ **它不決定那些人戰後在哪一郡**。原版的退兵常式（`0x23dd4`–`0x24460`）
	// 一個字都沒寫人物記錄 offset 19；去處只決定**從戰場的哪一格走出去**
	// （`0x241be` 拿它比對鄰郡表挑出口）。戰後的所在郡由**戰後安置**
	// （`0x1fb26`）決定，而那一支逐軍團、逐部隊、逐槽走**所有人**——
	// 退兵的那一支照樣在部隊記錄裡（退兵只清戰場佔位圖 `0x24400`）。
	// 所以這一格是給畫面與戰報用的，不進三張表（`docs/re/05` §12.3）。
	RetreatTo int
...
// RestMove 是休息一次增加的移動力（玩家 `0x27c27`、電腦 `0x29e53`，
// `L0`；說明書 p.29、p.30 也寫「每休息一次可增加移動力 2」）。
const RestMove = 2
