...
func (g *State) EndMonth() []Event {
	...
	// 原版在月底結算（`0x1582d`）與**每一個郡的回合入口**（`0x17471`）
	// 各做一次；這裡是前者，後者在 `session.runPrefectureTurns` 與月度
	// 對拍的逐郡迴圈裡。
	//
	// ⚠ **只做換月那一次是不夠的。** 電腦的移防會把整份出征名單搬走
	// （`docs/spec/007` R1），來源郡可能當場被搬空——那個郡如果排在
	// 順序表後面，原版在它的回合入口就已經把它算成無主而跳過，只在
	// 換月重算的話 remake 會讓一個空郡照跑十八張表。
	//
	// 少了這一步，電腦諸侯的出兵就完全沒有意義：原版的「出兵」只是把
	// 部隊搬進目標郡，郡易主全靠這次重算。
	g.RecomputeOwners()
	g.markPhase("換月")
	// **開月在四季之前。** 月底那一段是（`0x1581c`，`docs/re/06` §9.5）：
	//
	//	0x1582d  call 0x1e394     ; 重算所屬
	//	0x15831  call 0x17364     ; 開月：旗標、順序表、物價，最後洗五輪
	//	0x15835  call 0x159de     ; 月份推進
	//	0x15839  call 0x15c1e     ; 四季分派器 → 冬 0x16e70
	//
	// 亂數的帳也是這樣：原版從結算入口到第一個郡之間抽 376 次，
	// 拆開是物價 86 ＋ 洗牌 215 ＋ 人口成長 6 ＋ 民亂 2 ＋ 進貢 69。
	// 把物價或洗牌擺到四季後面，整條序列從第一個郡就對不上。
	//
	// 月份推進在這裡已經做過（函式開頭），順序不影響——開月那一支
	// 不讀月份，四季分派器讀的是推進後的值。
	g.repriceAll()
	g.markPhase("物價")
	// 洗牌的結果留在盤面上：下個月的郡順序是**這一刻**定下來的，
	// 原版連同旗標一起寫進進度檔（`docs/re/08` §2）。
	g.turnOrder = g.ShuffleTurnOrder()
	g.markPhase("洗牌")
	out := g.RunSeason()
	g.markPhase("四季")
	return out
