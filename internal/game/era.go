...
//
// 原版的 `1058:21A0`／`1058:23DC` 有 17 筆年號起始年表
//（`DS:0x6018`）與對應字碼對（`DS:0x603A`）；它選最後一個不大於
// 目前西元年的項目，再算「西元年 − 起始年 ＋ 1」。dosgolem 從
// 劇本一正常走過十二次月底後，畫面由「中平六年元月春」轉為
//「初平元年元月春」（`TestOriginalEraAfterOneYear`）。
