...
規則層共用是現況：`state.Edition`（`base`／`plus`）只切**已經量到的
差異**——難度上限（10／20）、電腦諸侯的出兵係數表、戰役勝負的統帥條件、
開局難度對 AI 等級的改寫、電腦諸侯十八張表裡動過的四張。規格在
`docs/spec/004` §6，證據在 `docs/mechanics/90` §6。
...
20. **位址型別要當成單位**：`o.ToIDA()` 的輸出是 **IDA 位址**，
    攔截點與 `objdump` 要的是**線性位址**，這一款差 `0xEF00`。
    兩者長得一模一樣（五位十六進位數），混用不報錯——只會讓反組譯落在
    別的函式上，**而 1991 年的碼密度高到隨便一段都讀得通**。印位址時
    兩個都印並標明是哪一個。露餡的訊號是**攔截次數**：真的是那支函式的
    入口，不可能一次都攔不到（`CONTEXT.md` R50）。
21. **hook 命中 0 次要先當成「攔錯地方」，不是「條件不成立」。**
    下任何「原版沒有做 X」的結論之前，先看那個 hook 有沒有命中過。
22. **註解裡的位址語意是二手推論。** 與一手的執行順序衝突時以順序為準——
    把可疑的位址和其他路標一起排一次時間軸，比讀十遍註解快
    （`0x174f3` 被標成「分派器之前」用了好幾輪，實際排在它之後）。
