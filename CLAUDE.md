...
**⚠ 這裡有一個必須先解釋的矛盾**：`DATA0.GRP` 長度翻了一倍，
而 `DATA0.IDX` 一個位元組都沒變。所以 `.IDX` **不會是單純的位元組位移表**，
否則長度變了它必然跟著變。可能是定長槽、可能索引的是別的東西、
也可能 `.GRP` 自己帶內部檔頭。**先解出 `.IDX` 到底索引什麼，再碰 `.GRP`**——
反過來做會得到「自洽但錯」的解碼器（`rulebook` 反覆踩過的形狀）。

`DATA2.GRP` 兩版長度相同、內容不同：這是**逐位元組 diff 最划算的一份**，
差異落點會直接指出加強版改了什麼。
...
說明書是**繁中原文的權威來源**：遊戲內用字、指令名、能力值名一律以它為準，
英日譯名從這份出發。但它的**規則與數值只是提示**，位階低於執行檔（§4）——
1991 年的說明書與最終出貨版本不一定一致，而且這份手冊是哪一版的還要確認。
...
規則層共用是**目標**，不是現況。目前只知道三個 `.GRP` 不同（§2.1），
不知道差在資料還是差在規則。

**在解出差異之前**：
- 不要預先造版本旗標。造出來的旗標會固定住一個還沒驗證的假設。
- 兩版都當一等公民抽檔、記雜湊，`workplace/orig/{base,plus}/` 分開放。
- 每一條機制斷言標明**在哪一版驗的**。跨版外推一律禁止（§7 第 9 條）。

差異解出來之後才決定旗標的粒度，寫進 `docs/spec/`。
...
18. **解壓／解碼錯一個位元組不會報錯**，只會讓多數數值碰巧是對的。
