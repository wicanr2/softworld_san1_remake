...
F15：42 個郡在手冊裡只有編號。郡名對照表**必須從遊戲資料取**，
這正是 `CLAUDE.md` §7 第 5 條的實例——三國題材下憑印象補郡名，
錯的東西會看起來非常合理。`.NAM` 三件套是第一個該找的地方。
...
| `docs/reference/` | 說明書整理、社群資料 | `01-manual-*` 完成（5 份）|
...
- [ ] 從資料檔找 42 個郡名（§3.06）與 342 位人物表，對回手冊附錄三
