...
| **配樂解出來了** | `MUS.GRP` 的五首（思古／小徑／風雲／戰鼓／末路）＋ AdLib 音色庫；曲子是 MIDI 事件流，事件數與表頭精準吻合（`docs/formats/06`）| 2026-09-06 |
...
| **多語系** | 介面文字收進 `internal/i18n/lang/*.json`，繁中（原文）／英／日；字型涵蓋率有測試盯著 | 2026-09-06 |
| **素材匯出** | `cmd/san1assets`：圖→PNG、劇本→JSON、配樂→MIDI，附來源雜湊的 manifest | 2026-09-06 |
...
- [ ] 解音色庫的 56 個位元組怎麼對到 AdLib 的參數 → 解出來才能做 OPL2 合成；
      **在那之前不要合成**，用錯的參數合出來的聲音是我們編的。
      方法已經確定（與點陣圖同一招）：錄放歌時的 OPL2 寫入序列來對
- [ ] 讓原版在 dosgolem 裡放得出音樂 → 現在選 AdLib 會吐
      `ErrMsg: AdLib / ErrNo: -1` 離開；偵測是過的，失敗在更後面
- [ ] 素材匯出加 OGG（要先有 OPL2 合成）
- [ ] 解 `MUSV` 那一首長曲少掉的六個事件（`TestLongSongIsStillOpen` 釘著）
