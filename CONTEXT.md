...
對拍要的不是整張表，是**這一局的那一組**——執行器是決定性的，
同一段開機序列每次都拿到同一組座標（地支 `寅`），答案固定。
那一組已經問出來了：**`4029`**。做法是拿原版自己的記憶體當來源
——它得先算出答案才畫得出那個畫面，所以答案一定在「主畫面 →
密碼畫面」之間變動的那 3,804 個位址裡（`internal/parity` 的
`TestZZPassword`）。
...
- [ ] 從執行檔／資料檔取 1728 組密碼表（§3.05）——**對拍已經不擋了**
      （這一局的那一組 `4029` 從記憶體問出來了）；整張表只剩考據價值
