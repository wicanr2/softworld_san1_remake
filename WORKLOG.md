...
## 2026-09-24：公開發行、README 與本機推廣片

- README 加強遊戲玩法介紹，從現行 `cmd/san1dump` 重生劇本 001 主畫面 `docs/images/remake-main.png`；從目前版本移除六張舊截圖。Git 歷史沒有改寫，舊截圖仍在歷史提交中。
- `tools/release.sh v.1.0.3-20260924` 從乾淨提交 `8d89946` 建立四平台公開引擎包，封包帶現行截圖，內容清單、壓縮、版號、型別與 SHA-256 通過。Linux 原版與加強版各啟動 25 秒。`tools/full-local.sh` 另從四包加入兩版 66 個原版檔，逐檔重讀 SHA-256，最終 Linux 完整包兩版各啟動八秒；本機完整版不公開。
- 推廣片首次以六路同步 FFmpeg 濾鏡合成時超出容器資源；改逐段編碼後可產片，但原選「思古」轉出 WAV 幾乎靜音（平均 −91 dB）。改採有聲的本機原版「風雲」並用逐幀縮放；正式 42 秒影片平均 −38.5 dB，無連續 0.5 秒黑幀或 2 秒凍結。六格接觸表已目視核對；影片、音訊、抽樣幀、檢測及 SHA-256 存於 `dist-all/v.1.0.3-20260924/promo/`，只留本機。
- `v.1.0.1-20260923` 與 `v.1.0.2-20260924` 的本機產物未覆寫；已把修正後來源另以同日修訂號 `v.1.0.3-20260924` 重建。推送 `main` 與同名 Git tag，建立正式 GitHub Release，遠端附件僅四個公開引擎包與公開雜湊清單；把儲存庫改為公開並回讀 `PUBLIC`。#102、#103 已完成關閉；#2 留言記錄未簽及缺少 Windows／macOS 原生驗收，保持開啟。
- 此次公開包不含原版執行檔或資料。含原版遊戲的四平台完整版，以及含原版美術、配樂的推廣片都只在本機，沒有上傳。諸侯表與戰術全程仍依 `docs/spec/020`、`docs/playtest/03` 揭露限制。
- 收尾重新計算四個公開包、四個本機完整版及影片 SHA-256，均與 `SHA256SUMS.json` 相同；公開清單只列四個引擎包。本輪 `dist-all/`、影片來源與打包工作目錄未見 root 擁有檔案或誤建的 `.md` 目錄。Docker 執行中容器的掛載檢查未發現本專案殘留；其他專案容器未處理。
