...
裡，並排成 640×400 貼上去；畫面是 640×408，底下 8 列是貼圖前清成的黑。
remake 照同一個擺法拼回去，與原版那一格 **261,120 格逐格相同**。
...
主選單。整張都是原版的素材：標題牌 `MENU0A`／`MENU0B`、左側直牌
`MENU1`、六個按鈕 `MENU2`、右下角小飾框 `MENU3`。這一張的程式在開機鏈
的第二層，主程式的碼段 dump 涵蓋不到，所以位置與字的版面全部是拿原版
的畫面逐像素比對出來的——**扣掉字與小飾框裡那格動畫之後，245,170 點
與原版逐點相同**。

字距也是量的：原版在每個中文字前面空一個半形格，所以字距是 24 不是 16。
字模是 remake 自己的字庫，不內嵌任何原版字模。

基準畫面本身另外拿 DOSBox-X 跑一次原版交叉驗證過（`tools/dosboxx.sh`），
兩個獨立實作扣掉那格動畫之後 **261,036 點逐點相同**（`docs/playtest/03`）。

畫面是 **640×408** 不是 640×350——原版寫進 CRTC 的 Vertical Display End
就是 408 列，主畫面的版面（上花邊 36 ＋ 直條 336 ＋ 下花邊 36）也剛好
是這個數字（`docs/spec/006`）。

不只一張。`tools/dosboxx-record.sh` 拿一份按鍵腳本驅動 DOSBox-X 跑完
開局 → 選主君 → 出兵宣戰 → 軍團整編，逐步存下原版的畫面；dosgolem 送
同一串鍵，一步一步比。列入判準的 32 張裡有 20 張在 99.6% 以上、
4 張逐點相同。對不上的都是**原版自己抽的東西**——防拷題目的地支
（一邊抽到「未」、另一邊「巳」）與一個宣戰事件；換一份腳本重跑，
對不上的區間會換位置，而且都自己收斂回來。

![接原版素材的主畫面](docs/images/main.png)

主畫面。**圖像來自原版**——地圖、花邊、標題牌、右側面板與人物肖像都是
原版的素材，執行時從玩家自己那一份讀進來，位置與原版相同
（`docs/spec/005`）：八張 `MAINMAP*` 拼成底圖，州郡從記錄裡的地圖座標
（`+0x50`／`+0x2c`）灌色，肖像畫在 (536,116)。配樂也是原版的，
從 `DATA1` 邊播邊合成。

右側那兩塊面板的花邊外框是**拼件平鋪**的：`DATA1` 裡有一組 32 塊
（8×8 與 16×16 各十六塊），四個角用 16×16、四條邊用 8×8。
兩塊面板的外框與原版 **12,544 點逐點相同**。面板上每一列的位置與字色
（郡名黃、州名洋紅、人望黃、欄位淺青、主事者姓名淺綠）也都是量的。
...
主戰場那一張是 `-screen artbattle -sel 8 -months 2`——郡要挑劇本裡湊得出
兩個真勢力的（`-sel` 預設的廬陵湊不出來，會退回自組的樣本局面，統帥
的姓名與肖像就是空的）。
...
| M8 發行 | 本機交付版號為 `v.1.0.0-20260923`；封包及驗收結果以 `dist-all/<版本>/SHA256SUMS.json` 為準；Windows／macOS 原生啟動與簽章仍待驗 |

正式交付以專案根目錄的 `dist-all/<版本>/` 為入口；建置與驗收方式見
[`發行包與本機驗收`](docs/release/01-packaging.md)。
