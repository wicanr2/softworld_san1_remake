...
同一個方法後來又找出加強版的行為觸發開機要的路標（Issue #20，整張表在
`docs/spec/015` §8.1）：數字輸入常式 `0x317e4`、Y/N 輸入 `0x144ba`、
郡回合入口 `0x16256`、掃描碼「等新鍵」同步點 `0x10bbd`。**形狀比對在
runtime 那一層有效、在主程式那一層失效**——加強版用 286 指令編譯，
`mov ax,imm / push ax` 全變成 `push imm8`，主程式的呼叫端得靠實跑攔。

月度對拍要的另一組路標（Issue #26，整張表在 `docs/spec/015` §8.4）
是拿**286 的形狀**比出來的：`RND(n)` 包裝（`0x103aa`，`cwd / idiv [bp+6] /
mov ax,dx` 那三道兩版相同）、十八張分派表的呼叫端（`shl bx,2 / call
dword [bx+表]`，原版是 `shl bx,1` 兩次）——表在 DS 的位移一律 +0x82。
亂數種子不用比：從 `rand()` 本體的 `push ds:[…]` 兩道直接讀出
（`DS:0xa566`／`0xa568`）。
...
## 6. 順帶量到：人望每年秋季會調整（`L0`；已入 `docs/mechanics/50` §4.1，Issue #18）
...
不同的係數**。已對拍並實作（`TestZZPrestigeAutumnDrift`、`game.PrestigeDrift`）。
