...
## 2.5 方向修正（2026-09-16 裁定）

規則層的對拍**到此為止**：兩版的月度對拍十二個視窗、玩家命令、戰役逐日、
對戰子畫面與單挑都已逐位元組／逐鏈相同（#44 收據），機制差不多了。
§3 的 **A／B／C 三軌停開新 Issue**（#45／#46／#47 已關 `not planned`，要回頭做時重開）。
迴圈改做**畫面一致性**：**原版該有人物頭像的畫面，remake 就該有**（#49 訊息框、
#50 九個畫肖像的呼叫端）；判準仍是逐像素／逐格，不是「看起來像」。
...
**F. 畫面一致性**（2026-09-16 起第一優先）：原版每一個畫面狀態畫了什麼（肖像、框、字級、落點），remake 同一狀態要畫一樣的。來源三處：畫肖像常式 `0xf7b4` 的 9 個呼叫端（#50）、訊息常式 `0x3273e`（#49）、`VERIFICATION-MATRIX.md` §2.5 標「remake 自排」「未接」的列（#48）。一個畫面一條，驗收是 dosgolem 存的原版基準畫面（DOSBox-X 交叉驗一張）與 remake 輸出逐像素／逐格比。

**A. 加強版對拍**（**停開**，2026-09-16）（`docs/mechanics` 107 條 `[base]`、1 條 `[plus]`）。先確認 `ASV.EXE` 能照 `docs/spec/015` 行為觸發開到主畫面，位址放 `internal/parity` 加強版自己的檔（如 `plus_oracle_test.go`）。再依序搬：開新遊戲盤面 → 月度轉移 → 十四項玩家命令 → 戰役逐日 → 存讀檔，一項一條。對完改標 `[both]`，差異進 `docs/mechanics/90`；**沒量到差異不造旗標**。

**B. 21 個 `Tune*` 換原版公式**（**停開**，2026-09-16）（`docs/design/02`）。一個常數一條：先 grep 使用處，死碼直接刪；活碼從 `.i64` 找原版算式（先 grep `docs/re/`），有就換掉對拍退役；原版無此規則則常數留著、`design/02` 標 remake 差異後關閉。

**C. 沒實跑過的規則**（**停開**，2026-09-16）：六種戰場計謀各一條（玩家親征、盤面直寫記憶體、下計謀、比原版部隊記錄）；語音 `R499` 語意（`docs/re/09` 的方法）。
