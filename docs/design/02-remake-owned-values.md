...
沒有 remake 自選的值。瘟疫的「將領體能下降」（p.36）在碼裡是
`RND(10)+70`% 的保留率（`0x1697c`），連兵也照 `RND(20)+40`% 掉
（`docs/re/06` §2）；水災同樣動郡裡每一位人物的兵（§1）。
...
## 5. remake 加在「其他」底下的三項

原版的「其他」只有八項（`docs/re/04` §3 的字串表）。remake 加了第九到
第十一項，**加在後面**——原版那八項留在原來的編號上，玩家的手指記得
「9-3 是音樂」。

| 第幾項 | 鍵 | 名稱 | 做什麼 |
|---|---|---|---|
| 9 | `9` | 電腦AI | 在 `base`／`plus`／`enhanced` 之間循環 |
| 10 | `0` | AI指令 | 強化 AI 一個郡一個月下幾道令（1–5，預設 1）|
| 11 | `A` | 守城 | 電腦來攻時誰指揮守方（Issue #64）|
...
- **第十一項起改用字母鍵。** 數字用完了，`ui.MenuKey` 第十一項回
  `'A'`＋序，`cmd/san1` 的按鍵迴圈**在子選單開著時**跟著掃 `A`–`Z`
  （主選單按字母仍然什麼都不做）。同一行還逼著把第十項的名稱從
  「電腦指令」縮成「AI指令」（日文「表示速度」縮成「速度」）——
  下面板只有 4 列 × 24 格，多一個項目就多一列，整個子選單排不進去
  （`TestSubMenusShowInFullInEveryLanguage`）。

「守城」切的是**誰指揮守方，不是規則**：原版被打時由玩家自己編隊、佈陣、
指揮（說明書 p.26–28，`docs/reference/01` §B），remake 的預設卻是自動打完——無畫面的路
（月度對拍、示範模式、批次跑）沒有人可以下令，預設要親自守的話
那幾條會停在戰場上等一個永遠不會來的鍵。整編、日迴圈、收尾兩邊
走同一條路（`game.ComputerAttack` → `PendingDefence` → `FinishDefence`）。

「AI指令」調的是**強化 AI 有多強，不是規則**：原版的電腦本來就不受
