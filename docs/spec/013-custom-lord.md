# 013：自創君主

狀態：`READY`

範圍：開新局時選「新君主欄」那一條路——資料在哪、寫什麼、remake 的
介面做到哪裡。

證據等級：`L0`（範本、槽位、欄位版面）＋ `L2`（手冊的 100 點與空白郡）
＋ `L3`（點數怎麼花）。版本：`[base]`。

## 1. 新君主欄是劇本裡本來就有的（`L0`）

劇本 001 的十六個諸侯槽裡，槽 14 與 15 的君主欄指向人物 346／347
（填充筆），而且一個郡都沒有——那兩個就是新君主欄。手冊 p.7 寫
「16（含 2 個新君主欄）」，與資料完全對得上。

判準是 `Scenario.CustomLordSlots()`：**君主欄指向範本（人物 346 起，有號
比較）而操縱方是電腦的槽**——原版開新局的判斷（`0x123b6` 的
`cmp es:[bx+2],0x15a`，`docs/mechanics/80` §3）就是「君主槽 ≥ 346 而沒被
玩家選成自創君主」，玩家選了的那一格操縱方會寫 1。劇本一的空欄在
14／15，二在 15，三是 4／5／10／11，四是 10–13，五是 4／5，六是 4–7
（`TestCustomLordSlotsAcrossScenarios`）；三到六的範本槽夾在中間、
操縱方是 2，「不在 `ActiveFactions` 裡」那條舊判準在那四個劇本一個都
列不出來（Issue #25）。`ActiveFactions` 同一條：範本配電腦不算在用。


## 2. 範本（`L0`）

`AA.EXE` 的 `0x3e44c` 有四筆 30 byte 的人物記錄（`docs/re/08` §3）：

| 名額 | 姓名碼 | 年齡 | 體能 | 謀略 | 戰力 | 魅力 | 肖像 |
|---|---|---|---|---|---|---|---|
| 1 | `A141 A142 A143` | 20 | 90 | 60 | 80 | 90 | 11 |
| 2 | `A144 A145 A146` | 20 | 90 | 60 | 80 | 90 | 1 |
| 3 | `A147 A148 A149` | 20 | 90 | 60 | 80 | 90 | 15 |
| 4 | `A14A A14B A14C` | 20 | 90 | 60 | 80 | 90 | 9 |

`A141`–`A14C` 在標準 Big5 是全形標點；原版把它們挪用成**造字**，
字模隨進度存在 `BASEPRE.SVn`（出貨的內容是「新君主」三個字重複四次）。

## 3. 寫進去什麼（`L0` 的欄位版面）

`Scenario.WithCustomLord(faction, lord)` 回傳一份新的劇本，原件不動：

```
人物槽（諸侯表 offset 2 指的那一位）
  0–5   姓名 ＝ 三個造字碼位（A141 + 名額×3）
  7     年齡 20            8–11  四項能力
  13    出身郡 ＝ 領地      16    忠誠 100
  17    身分 0（君主）      18    勢力
  19    所在郡 ＝ 領地      27    肖像（照名額）
州郡表那一郡
  30    所屬 ← faction      32    主事者 ← 那個人物槽
```

**諸侯表不動**：君主欄本來就指著那一筆，其餘 70 個位元組還沒解
（`docs/spec/003`），原封不動比填一個猜的值安全。

## 4. 點數（`L2` ＋ `L3`）

手冊 p.14：「每位新加主有 **100 點基數**，修改二、三、四、五項均需使用
點數」「領地也只有**一郡**，而且**必須是空白郡**」。

remake 的規則：**四項能力從範本的底（90／60／80／90）往上加**，
一點加一，各自夾在 100；分配不完可以直接完成。

⚠ **`L3`**：手冊沒寫加一點花幾點、也沒寫上限。範本的四項加起來是 320，
與「100 點基數」不是同一個數字，所以 100 點只能是**額外**可分配的。
上限取 100 是照一般能力值的值域推的。

**這一段在主程式裡，不在 `DATA0.GRP`**：分配能力那一格是 `0x12df4`
（`docs/spec/005` §9.5，`L0`）——進去先把範本改寫成年齡 18、体能 80、
謀略／戰力／魅力 50、100 點，領地預設槽號最大的空白郡；點數怎麼花在
輸入常式 `0x13524`（還沒讀）。remake 的底（20、90／60／80／90）因此
與原版開局時的值不同，規則層對齊另開 Issue。

## 5. remake 這一側

### R1 選角色那一層列出新君主欄

排在真的諸侯後面。不列的話玩家永遠選不到（`internal/menu`）。

### R2 設定那一層是六項，不是七項（**remake 差異**）

| 原版（手冊 p.14）| remake |
|---|---|
| 1 年齡 | **照範本 20**，不給改 |
| 2–5 體能／謀略／戰力／魅力 | 左右鍵加減，共用 100 點 |
| 6 領地 | 左右鍵在空白郡之間換 |
| 7 修改姓名（用畫的）| **照範本「新君主」**，不給改 |

年齡與姓名沒做：年齡的提示是「您的年齡\n(8-72):」（`DS:0x628c`，`L0`），
花法在 `0x13524` 還沒讀；姓名在原版是一點一點畫出來的。範本是 `L0`，
照它填不會錯。

原版素材畫面的這一層畫成原版的樣子（肖像加框、名字 32×32、六行）：
`docs/spec/005` §9.5。

### R3 空白郡是硬條件

一個都沒有時當場說出來，不能讓玩家設定完才發現開不了局。

### R4 名字的字模跟著這一局走

原版把自創君主的名字當**造字**（Big5 `A141`–`A14C`），字模存在
`BASEPRE.SVn`——人物表裡只有碼位，沒有字模就是三個空白。

remake 開自創君主的局時，把**原版出貨的那一份 `BASEPRE`** 帶進來
（`DATA2` 的 `BASEPRE.SV1`；六個進度位元組完全相同，內容就是「新君主」
三個字重複四次，`docs/re/08` §3）。存檔寫它、讀檔讀回來。

**一般君主的局不帶**：那一局沒有造字要畫，存檔那一層會沿用上一次寫出去
的——存讀一輪不該把別人畫好的字洗掉。

## 6. 驗收

| 項 | 怎麼驗 |
|---|---|
| 劇本 001 有兩個新君主欄 | `internal/state` 的 `TestCustomLordSlotsMatchTheManual` |
| 六個劇本的新君主欄各對上君主槽 ≥ 346 的那幾格 | `TestCustomLordSlotsAcrossScenarios`、`internal/menu` 的 `TestNewLordSlotsInScenarioThree` |
| 範本的初始值 | `TestCustomLordStatsStartFromTheTemplate` |
| 點數、空白郡、姓名的檢查 | `TestCustomLordValidate` |
| 寫進去讀得回來、原件不動 | `TestWithCustomLordWritesTheTables` |
| 整條流程走得完 | `internal/menu` 的 `TestPicksACustomLord` |
| 字模帶得進去、存讀活得下來 | `TestCustomLordCarriesTheShippedGlyphs`、`internal/save` 的 `TestGlyphsSurviveASaveLoadRound` |

## 7. 還沒解的

| 缺口 | 現況 |
|---|---|
| 點數怎麼花、上限多少 | §4 的 `L3`；起始值 `0x12df4` 已讀（18、80、50／50／50、100 點），花法在 `0x13524` |
| 年齡怎麼收 | R2 |
| 姓名怎麼畫 | R2；名字固定「新君主」。要讓玩家改名就得自己產字模（從 remake 的字庫取），那一步還沒做 |
| 玩家人數（多人）| 原版有，remake 沒做（`worklist.json` 的 `multiplayer`）|
