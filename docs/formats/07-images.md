...
內容是一條山巒的輪廓（下方全 0、上方全 1）。

**它是 `REC10`（山城風景，640×336）的天空遮罩**：把遮罩的 1 當天空、
0 當山，與圖裡「這一格是不是天空色（11）」逐點比，**位移 y ＝ 54 時
吻合 99.39%**；同一個判準對另一張山景 `REC11` 只有 51.31%
（正對照，`docs/spec/012` §2）。

用途：製作群的字幕從**山後面**升起來——字只在天空那一側畫得出來
（`docs/spec/012`）。⚠ **怎麼合成是推測**：那一段跑在 `DATA0.GRP`／
`DATA4.GRP` 兩支 overlay，主程式的映像裡一次都沒出現 `ENDO`／`UPR`／
`REC1` 這些字串。
