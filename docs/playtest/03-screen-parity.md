...
- **時間軸不能照抄。** DOSBox 用真實秒數（`cycles=fixed 60000` ＝ 每秒
  六千萬個週期），dosgolem 的時鐘是指令數。腳本裡的秒數要換算
  （約每秒三千萬道指令），**給少了畫面會停在動畫中間，看起來像
  dosgolem 少做了什麼**。
- **只等「畫面靜止」不夠，要先跑滿預算再等靜止。** 開場是好幾段動畫接
  起來的，第一次停格就回來的話，下一個鍵會送在還沒走完的畫面上
  ——之後每一步都對不上，**而每一步看起來都「靜止」**。
- **兩條輸入路徑要分開餵**（`docs/re/02` §3）：裝置三題走 `int 21h`，
  選單走硬體掃描碼，防拷密碼兩條都要。每一步之前先 `Drain()`
  ——沒清乾淨的字元副本會把下一個提示堵死，而那看起來像「按了沒反應」。

剩下對不上的都是**會動的小東西**：主選單右下角那格飾框的動畫
（`x 592..599`）、以及提示列尾巴那個閃爍的游標（`x 576..583`）。
兩個實作不會停在同一格，那不是誰做錯了。

### 判準只能落在「畫面靜止」的那幾步

錄製時每一步存兩張、隔一秒；兩張一樣才算靜止。開場動畫那幾步兩個實作
不會停在同一格（DOSBox 用真實秒數、dosgolem 用指令數），
**那不是誰做錯了**，所以不列入判準。
